...
export async function onRequestGet(context) {
  ...
  const [osR, routeR, sealPolicyR, recordVerifyPolicyR] = await Promise.all([
  ...
  return json({
    schema: "speedkit.public_control_release_seal_response.v1",
    status: sealed ? "PUBLIC_CONTROL_RELEASE_SEALED" : "PUBLIC_CONTROL_RELEASE_SEAL_DEGRADED",
    mode: "PUBLIC_ONLY",
    fake_checkout: false,
    sealed,
    release_seal: {
      id: "speedkit-release-seal-" + String(releaseRecordVerificationHash || releaseSealHash).slice(0, 16),
      hash_algorithm: "SHA-256",
      release_seal_hash: releaseSealHash,
      release_seal_hash_excludes_sealed_at: true,
      material_schema: releaseSealMaterial.schema,
      release_record_verification_hash: releaseRecordVerificationHash,
      release_record_hash: releaseRecordHash,
      final_attestation_verification_hash: finalAttestationVerificationHash,
      attestation_hash: attestationHash,
    ...
    failures,
    invariants,
    release_seal_material: releaseSealMaterial,
    ...
    source_status: releaseSealMaterial.source_status
  }, sealed ? 200 : 503);
}
