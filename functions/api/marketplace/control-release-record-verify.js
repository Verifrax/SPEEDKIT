...
export async function onRequestGet(context) {
  ...
  const releaseRecordFailures = Object.entries(releaseRecordInvariants)
    .filter(([, ok]) => ok !== true)
    .map(([key]) => key);
  ...
  const verifierInvariants = {
    ...
    release_record_verify_policy_live: policy.status === "LIVE"
  };
  ...
  return json({
    schema: "speedkit.public_control_release_record_verify_response.v1",
    status: verified ? "PUBLIC_CONTROL_RELEASE_RECORD_VERIFIED" : "PUBLIC_CONTROL_RELEASE_RECORD_VERIFY_FAILED",
    mode: "PUBLIC_ONLY",
    fake_checkout: false,
    verified,
    ...
    invariants: verifierInvariants,
    source_status: {
      release_record: releaseRecordFailures.length === 0
        ? "PUBLIC_CONTROL_RELEASE_RECORDED_BY_LOCAL_REPLAY"
        : "PUBLIC_CONTROL_RELEASE_RECORD_REPLAY_FAILED",
      final_attestation_verify: finalAttestationVerifyFailures.length === 0
        ? "PUBLIC_CONTROL_FINAL_ATTESTATION_VERIFIED"
        : "PUBLIC_CONTROL_FINAL_ATTESTATION_VERIFY_FAILED",
      final_attestation: finalAttestationFailures.length === 0
