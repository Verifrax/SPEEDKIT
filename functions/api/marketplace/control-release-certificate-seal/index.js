const certificateVerify = {
  ...
  "status": "PUBLIC_CONTROL_RELEASE_CERTIFICATE_VERIFIED",
  "mode": "PUBLIC_ONLY",
  "fake_checkout": false,
  "verified": true,
  ...
  "failures": [],
  "invariants": {
    "release_certificate_snapshot_json": true,
    "release_certificate_status_certified": true,
    "release_certificate_hash_present": true,
    "release_certificate_hash_ok": true,
    "release_certificate_failures_zero": true,
    "release_archive_verification_hash_expected": true,
    "release_archive_hash_expected": true,
    "release_archive_recomputed_hash_matches": true,
    "release_archive_hash_ok": true,
...
export async function onRequestGet() {
  try {
    const sealRouteCount = 123;

    const certificateVerificationHash = certificateVerify.verification_hash || null;
    const releaseCertificateHash = certificateVerify.verification?.release_certificate_hash || null;
    const recomputedReleaseCertificateHash = certificateVerify.verification?.recomputed_release_certificate_hash || null;
    const releaseArchiveVerificationHash = certificateVerify.verification?.release_archive_verification_hash || null;
    const releaseArchiveHash = certificateVerify.verification?.release_archive_hash || null;
    const releaseSealVerificationHash = certificateVerify.verification?.release_seal_verification_hash || null;
    const releaseSealHash = certificateVerify.verification?.release_seal_hash || releaseCertificate.release_certificate?.release_seal_hash || null;

    const invariants = {
      certificate_verifier_snapshot_json: certificateVerify.schema === "speedkit.public_control_release_certificate_verify_response.v1",
      certificate_verified: certificateVerify.status === "PUBLIC_CONTROL_RELEASE_CERTIFICATE_VERIFIED" && certificateVerify.verified === true,
      certificate_verifier_failures_zero: Number(certificateVerify.failures_count ?? -1) === 0 && Number(certificateVerify.verification?.failures_count ?? -1) === 0,
      certificate_verification_hash_expected: certificateVerificationHash === "01a414f217a1169446871181bcc9591bd09428b61b30526d3ee758e9835a8e38",
      release_certificate_hash_present: /^[a-f0-9]{64}$/.test(String(releaseCertificateHash)),
      ...
      release_certificate_hash_ok: releaseCertificateHash === recomputedReleaseCertificateHash && certificateVerify.verification?.release_certificate_hash_ok === true,
      release_certificate_hash_matches_certificate_response: releaseCertificateHash === releaseCertificate.release_certificate?.release_certificate_hash,
      release_certificate_response_certified: releaseCertificate.schema === "speedkit.public_control_release_certificate_response.v1" && releaseCertificate.status === "PUBLIC_CONTROL_RELEASE_CERTIFIED" && releaseCertificate.certified === true,
      release_certificate_response_failures_zero: Number(releaseCertificate.failures_count ?? -1) === 0,
      ...
      release_archive_hash_ok: certificateVerify.verification?.release_archive_hash_ok === true,
      ...
      private_remaining_zero: certificateVerify.verification?.private_remaining === 0 && releaseCertificate.release_certificate?.private_remaining === 0,
      workspace_entries_135: certificateVerify.verification?.workspace_entries === 135 && releaseCertificate.release_certificate?.workspace_entries === 135,
      recognized_execution_systems_4: certificateVerify.verification?.recognized_execution_systems === 4 && releaseCertificate.release_certificate?.recognized_execution_systems === 4,
      certificate_verifier_route_count_floor: Number(certificateVerify.verification?.route_count ?? 0) >= 120,
      certificate_route_count_floor: Number(certificateVerify.verification?.certificate_route_count ?? 0) >= 117 && Number(releaseCertificate.release_certificate?.route_count ?? 0) >= 117,
      archive_verifier_route_count_floor: Number(certificateVerify.verification?.archive_verifier_route_count ?? 0) >= 114,
      archive_route_count_floor: Number(certificateVerify.verification?.archive_route_count ?? 0) >= 111,
      seal_route_count_floor: sealRouteCount >= 123,
    ...
    const sealMaterial = {
      ...
      release_certificate_verification_hash: certificateVerificationHash,
      release_certificate_hash: releaseCertificateHash,
      recomputed_release_certificate_hash: recomputedReleaseCertificateHash,
      release_certificate_hash_ok: releaseCertificateHash === recomputedReleaseCertificateHash,
      ...
      route_count: sealRouteCount,
      certificate_verifier_route_count: certificateVerify.verification?.route_count || null,
      certificate_route_count: releaseCertificate.release_certificate?.route_count || null,
      archive_verifier_route_count: certificateVerify.verification?.archive_verifier_route_count || null,
      archive_route_count: certificateVerify.verification?.archive_route_count || null,
      workspace_entries: 135,
      private_remaining: 0,
      recognized_execution_systems: 4,
      source_status: {
        release_certificate_verify: certificateVerify.status,
    ...
    return json({
      ...
      status: sealed ? "PUBLIC_CONTROL_RELEASE_CERTIFICATE_SEALED" : "PUBLIC_CONTROL_RELEASE_CERTIFICATE_SEAL_FAILED",
      mode: "PUBLIC_ONLY",
      fake_checkout: false,
      sealed,
      release_certificate_seal: {
        id: "speedkit-release-certificate-seal-" + String(sealHash).slice(0, 16),
        hash_algorithm: "SHA-256",
        release_certificate_seal_hash: sealHash,
      ...
      failures,
      invariants,
      release_certificate_seal_material: sealMaterial,
      release_certificate_verification: {
        verification_hash: certificateVerificationHash,
        release_certificate_hash: releaseCertificateHash,
        recomputed_release_certificate_hash: recomputedReleaseCertificateHash,
        release_certificate_hash_ok: releaseCertificateHash === recomputedReleaseCertificateHash,
        failures_count: Number(certificateVerify.failures_count ?? -1)
      },
    ...
    return json({
      schema: "speedkit.public_control_release_certificate_seal_response.v1",
      status: "PUBLIC_CONTROL_RELEASE_CERTIFICATE_SEAL_ERROR",
      sealed: false,
      failures_count: 1,
      failures: ["release_certificate_seal_exception"],
